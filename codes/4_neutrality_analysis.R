...
for(j in 1:length(Species))
{
  ...
  Neutrality.stats <- tibble(Species = Species[j], N = number_samples, NSS = seg_sites, 
                             Pi = Nucleotide_Diversity, Hd = Haplotype_Diversity, 
  ...
  if(!file.exists(per_species_stats)){
    ...
    write.table(Neutrality.stats, per_species_stats, append = TRUE, col.names = FALSE,
                row.names = FALSE, quote = FALSE, sep = "\t")
